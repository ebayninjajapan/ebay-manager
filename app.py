...
# ─────────────────────────────────────────
def sync_to_github():
    """CSVファイルの変更をGitHubに自動コミット・プッシュする"""
    # Streamlit Secrets から設定を取得
    if "GITHUB_TOKEN" not in st.secrets:
        # シークレットが設定されていない場合はスキップ（ローカル実行時など）
        return

    try:
        token = st.secrets["GITHUB_TOKEN"]
        repo_url = st.secrets["REPO_URL"] # 例: https://github.com/username/repo.git
        user_name = st.secrets.get("GIT_USER_NAME", "Streamlit Auto-Bot")
        user_email = st.secrets.get("GIT_USER_EMAIL", "bot@example.com")
        ...
        # 変更をステージング
        subprocess.run(["git", "add", DB_FILE, WATCH_FILE], check=True)
        ...
        # コミットとプッシュ
        commit_msg = f"Auto-update data: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        subprocess.run(["git", "commit", "-m", commit_msg], check=True)
        subprocess.run(["git", "push", "origin", branch], check=True)
        st.toast("GitHubへデータをバックアップしました ☁️")
    ...
    if not st.session_state.w_df.empty:
        ...
        edited_w_df = st.data_editor(
            ...
            column_config={
                ...
                "狙う仕入れ価格": st.column_config.NumberColumn("狙い価格(円)", format="¥%d"),
